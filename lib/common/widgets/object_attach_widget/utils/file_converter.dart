...
class FileConverter {
  ...
  ObjectAttach fileToObjectAttach(File file, int objectId) {
    return ObjectAttach(
        ...
        fileName: file.path,
        filePath: file.path,
        attachmentData: base64.encode(file.readAsBytesSync()),
        createDate: DateTime.now(),
        workerName: authController.authState.userInfo.value!.workerName ??
            "Неизвестен");
  }
  ...
  Future<ObjectAttach> fileXToObjectAttach(XFile xFile, int objectId) async {
    ...
    return ObjectAttach(
        ...
        fileName: xFile.name,
        filePath: xFile.path,
        attachmentData: base64.encode(fileData),
        createDate: DateTime.now(),
        workerName: authController.authState.userInfo.value!.workerName ??
            "Неизвестен");
  }
  ...
  Future<String> _createFileFromString(ObjectAttach objectAttach) async {
    ...
    File file = File("$tempPath/" + objectAttach.fileName);
