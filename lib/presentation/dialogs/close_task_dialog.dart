  ...
  Widget build(BuildContext context) {
    return GetBuilder<TaskListController>(builder: (taskListController) {
        ...
        buttonBar = ElevatedButton(
          ...
          onPressed: () async {
            if (_closeCode == null) {
              ...
              try {
                Task newTask = await taskListController.closeOrder(_closeCode!);
                this.setState(() {
                  _operationCompleted = true;
                  _error = null;
                  // TODO: временно необходимый параметр, пока не получаем исполнителей по закрытой задаче
                  _workers = taskListController.taskListState.currentTask.value!
                      .getAssigneeListToText(true);
                  taskListController.taskListState.currentTask.value = newTask;
      ...
      return AdaptiveDialog(
          ...
          titleText: !_operationCompleted ? "Закрытие наряда" : "Наряд закрыт",
          body: body,
          buttonBar: buttonBar);
    });
  }
