          ...
          onPressed: () async {
            DateTime? _startDateTime =
                (_startDate != null && _startTime != null)
                    ? new DateTime(_startDate!.year, _startDate!.month,
                        _startDate!.day, _startTime!.hour, _startTime!.minute)
                    : null;
            DateTime? _endDateTime = (_endDate != null && _endTime != null)
                ? new DateTime(_endDate!.year, _endDate!.month, _endDate!.day,
            ...
            } else if (_idleTime != null && _endDateTime == null) {
              this.setState(() {
                _error = "Укажите дату и время окончания простоя";
              });
            } else if (_startDateTime.isAfter(DateTime.now())) {
              this.setState(() {
                _error =
                    "Дата начала простоя должна быть не позднее текущей даты";
              });
            } else if (_endDateTime != null &&
                _endDateTime.isAfter(DateTime.now())) {
              this.setState(() {
                _error =
                    "Дата окончания простоя должна быть не позднее текущей даты";
              });
            } else if (_endDateTime != null &&
                _startDateTime.isAfter(_endDateTime)) {
              this.setState(() {
                _error =
                    "Дата начала простоя должна быть не позднее даты завершения простоя";
              });
            } else {
              ...
              if (_idleTime == null) {
                try {
                  ...
                  if (newIdleTime != null) {
                    ...
                    this.setState(() {
                      _error = "Неожиданная ошибка: простой не зарегистрирован";
                    });
                  }
                } catch (e) {
                  this.setState(() {
